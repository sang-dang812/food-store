                                ...
                                <div class="row">
                                    <div class="col-xs-12 col-sm-12 col-lg-8">
                                        <form method="post" action='dishes.php?res_id=<?php echo $_GET['res_id'];?>&action=add&id=<?php echo $product['d_id']; ?>'>
                                            ...
                                            <div class="mt-3">
                                                <?php
                                                    $stmt = $db->prepare("select * from comments where dish_id='".$product['d_id']."' ORDER BY time_stamp DESC limit 5");
                                                    $stmt->execute();
                                                    ...
                                                    if (!empty($comments)) {
                                                        ...
                                                        {
                                                            $stmt1 = $db->prepare("select * from users where u_id='".$comment['user_id']."'");
                                                            $stmt1->execute();
                                                            $users = $stmt1->get_result();
                                                            foreach($users as $user)
                                                            {                                                              
                                                             echo '<div class="card text-bg-secondary" style="max-width: 18rem;">
                                                                    <div class="card-header">'.$user['username'].'</div>
                                                                    <div class="card-body">
                                                                        <p class="card-text">'.$comment['cmt_text'].'</br> time: '.$comment['time_stamp'].'</p>
                                                                    </div>
                                                                    </div>';
                                                            }
                                                        }
                                    ...
                                    <div class="col-xs-12 col-sm-12 col-lg-3 pull-right item-cart-info">
                                        ...
                                        <input type="submit" class="btn theme-btn mb-1" style="margin-left:40px;" value="Add To Cart" />
                                    </div>
                                </form>
                                <form action="comment.php?res_id=<?php echo $_GET['res_id']; ?>" method="POST">
                                    <input type="hidden" name="dish_id" value = <?php echo $product['d_id'] ?>>
                                    <textarea name="comment" rows="2" style="margin-right:83px;" required></textarea>
                                    </br>
                                    <input type="submit" class="btn theme-btn"  value="Write comment" />
                                </form>
                                </div>
